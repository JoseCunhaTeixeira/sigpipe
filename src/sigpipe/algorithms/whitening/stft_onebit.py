...
def whiten_stft_onebit(
    ...
    *,
    ...
    if fft_size < 2:
        raise ValueError(
            f"fft_size too small ({fft_size}). Increase window duration or sampling frequency"
        )
    if fft_size > stream.nt:
        raise ValueError(f"fft_size ({fft_size}) is larger than signal length nt ({stream.nt})")
    _, _, data_fft = stft(stream.xt, nperseg=fft_size)
    if data_fft.size == 0:
        raise ValueError("STFT returned empty result. Check input signal and parameters")
