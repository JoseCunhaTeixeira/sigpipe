...
from sigpipe.dataio.velocity_model.saving import save_velocity_model
...
def save_inversion_result(result: InversionResult, path: Path) -> None:
    ...
    save_velocity_model(result.smooth_best, path.with_name(f"{path.stem}_smooth_best{path.suffix}"))
    save_velocity_model(result.median, path.with_name(f"{path.stem}_median{path.suffix}"))
    save_velocity_model(
        result.smooth_median, path.with_name(f"{path.stem}_smooth_median{path.suffix}")
    )
